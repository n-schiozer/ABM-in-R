...
name <- function_faster_labor_market_2 <- function(lista_funcionarios,firm_labor_demand,workers_empregador,firmk_labor_demand,workers_salario,firm_wage_ref){
  ...
  if(qtidade_firmas_contratando > 0){ # Ciclo admissão
    ...
    ms_applicants <- exp(3 * firms_wages_ref/mean(firms_wages_ref))/sum(exp(3 * firms_wages_ref/mean(firms_wages_ref)))
    
    
    # ATENÇÃO! COLOQUEI UM ZERO ABAIXO, SEM CHANCE DE TRABALHADOR MUDAR DE EMPRESA.
...
#firm_E <- ( 1 + effect.qual.mk * ( quality - quality_2 )/agg_quality - effect.price.mk * ( price - price_2)/agg_price - 
#              effect.pricevar.mk * ( sqrt(( price/price_2 - 1 )^2) ) -  effect.delay.mk * (delay/vendas) ) * LAG(firm_E,1)


#firm_E <- (quality/agg_quality)/( (price/agg_price) * (delay/vendas + 1) * ( 1 + sqrt(( price/price_2 - 1 )^2)) )

#fitness <- - effect.price.mk * (price/agg_price - 1) + effect.qual.mk * ( quality/agg_quality - 1) # -
#  effect.pricevar.mk * ( sqrt(( price/price_2 - 1 )^2) ) -  effect.delay.mk * (delay/vendas)

#average_fitness <- sum2( fitness * LAG(firm_ms,2) )

#firm_E <- lag_firm_E * ( 1 + fitness )

# Rotina para o caso de empresa quebrada:

#firm_quality <- ifelse(LAG(firm_leverage,1) > 1 | LAG(firm_ms,1) == 0 , mean(firm_E) - sd(firm_E)  , firm_E) # Se a empresa tiver quebrado no período anterior, a qualidade é a média 


#firm_E <- ( (quality/agg_quality)^effect.qual.mk )/( (price/agg_price)^effect.price.mk *
#                                                     (delay/vendas + 1)^effect.delay.mk * ( 1 + sqrt(( price/price_2 - 1 )^2) ) )
...
# firm_E <- exp(effect.qual.mk * log(quality/agg_quality) - effect.price.mk * log(price/agg_price) -
#                 effect.delay.mk * log(delay/vendas + 1))
# 
# firm_E <- exp(effect.qual.mk * log(quality) - effect.price.mk * log(price) -
#                 effect.delay.mk * log(delay/vendas + 1))
...
firm_ms <- ms * ( 1 - var.change.ms) + var.change.ms * exp(firm_E)/sum2( exp(firm_E) )
...
for(i in 1:quantidade_firmas){
  ...
  demand <- machine_demand[i]  - firm_machines[i] + num_firm_deprec[i] # Demanda atual de máquinas - máquionas existentes - máquinas que vão depreciar
  
  if(demand > 0){ # Se manda >0, entra um pedido novo na lista
    #if( any( list[1,] == i) == FALSE ){ # Se não houver pedidos da empresa ja na lista
    #if( firm_leverage[i] < 1 ){
    
    demand[ firm_leverage[i] > firm_leverage_meta[i] ] <- floor(0 * demand[ firm_leverage[i] > firm_leverage_meta[i] ])  # SE a alavancagem acima da meta, decisão de investimento é adiada
    
    #demand <- floor(( 1- firm_leverage[i])*demand)
...
#firm_custo <- VAR(firm_custo)
firm_salario_medio <- VAR(firm_custo_unit) # Custo unidade de trabalho por funcionário

firm_price_lag <- LAG(firm_price,1)

firm_demand_labor_ <- VAR( firm_labor_demand) # Demanda total necessária p/ antender a demanda esperada

demand_esperada <- VAR(firm_expdem)

# Custo total da firma. Conta tanto o custo do trabalho direto quanto o trabalho pelo tamanho da firma
# A demanda de trabalho é para que a empresa tenha a capacidade de produção desejada.

CT <- firm_demand_labor_ * firm_custo_unit 

# Produção normal: 

Producao.Normal <- VAR(firm_capacidade_producao)*grau_utilizao_desejado

#  Produção total que será executada pela firma se ela tiver o estoque total de trabalhadores necessários:

prod_total <- VAR(firm_expdem)/grau_utilizao_desejado #* (1 + firm_estoque_desejado_perc) # - LAG(firm_estoque,1)

# Total de trabalhadores necessários para produzir o desejado pela firma:


#trabalhadores_nece_eq <- ceiling( (prod_total/prod_machine) * parameter_trab_indireto + prod_total/(prod_machine * rel.k.l) )

trabalhadores_nece_eq <- VAR(firm_labor_demand)

# Posso usar duas medidas alternativas de custo, considerando apenas o custo do trabalho.
# Custo normal: qual é o custo quando a produção é a normal
# Custo cíclico: dando o ciclo dos estoques e demanda por trabalho, independemente da minha capacidade.


# Custo total para produzir o necessário:

CT_EQ <- trabalhadores_nece_eq * VAR(firm_wage_ref)

#CT <- (VAR(firm_coef_tecn) * VAR(firm_expdem)) * 1.1 * firm_salario_medio

CT <- trabalhadores_nece_eq * firm_salario_medio


#sd(CT/VAR(firm_expdem)/mean(CT/VAR(firm_expdem)))
#sd(CT_EQ/VAR(firm_expdem)/mean(CT_EQ/VAR(firm_expdem)))

#firm_custo_unit <- CT/VAR(firm_expdem)

FIRM_CUSTO <- LAG(firm_custo,1)
FIRM_CUSTO[ FIRM_CUSTO == 1 ] <- mean( FIRM_CUSTO[ FIRM_CUSTO != 1])

firm_custo_unit <- VAR(firm_wage_ref)*VAR(firm_coef_tecnico)/0.8

firm_custo[,1]/firm_vendas[,1]

mean(workers_salario[,1])
mean(firm_wage_ref[,1])

# L = Coef Y'2
# wL = w Coef Y/u
# wl/Y = w * Coef/u = Custo Unit
...
#firm_custo_unit <- (parameter_trab_indireto/0.8 + 1/(0.8 * 2)) * firm_salario_medio

#firm_custo_unit <- rep( sum2(CT_EQ)/sum2(VAR(firm_expdem)) , quantidade_firmas)

#firm_custo_unit/firm_custo_unit_EQ

# Custo por unidade esperada de demanda

#firm_custo_unit <- (CT)/demand_esperada

# Preço desejado e regra de preço adotada 

price_desejado <- LAG(firm_price,1)
price_rule <- VAR(firm_price_rule)

# Puxa informação do total produzido, pq, se for 1, adota a regra 3 
#firm_produzido <- VAR(firm_producao)
#firm_produzido[ firm_produzido < 1] <- 1
#price_rule[ firm_produzido == 1 ] <- 3


inflation <- LAG(agg_inflation,1)
#firm_ms <- LAG(firm_ms,1)

lag_price_dej <- rep( sum2( firm_price_lag * LAG(firm_ms,1)), quantidade_firmas )

firm_price <- LAG(firm_price,1)

# Para que rode primeiro a selecão
VAR(time_s)

lista <- list_firms_selected

for(i in 1:quantidade_firmas){
  
  lag_price_dej[i] <- sum2(firm_price[ lista[i,] ])/length(lista[i,] )
  
}

firm_grau_utilizacao <- LAG(firm_grau_utilizacao,1)

#firm_mkup.movel <- VAR(firm_mkup.movel)

# Funções p/ obter market-share relativo

#average_ms <- mean.default(firm_ms)

#relative_ms <- ( LAG(firm_ms,1) - LAG(firm_ms,2))/LAG(firm_ms,2)

# Preços desejados para cada uma das diferentes regras


price_desejado[ price_rule == 1] <- ( 1 + VAR(firm_fixed.markup)[ price_rule == 1])*firm_custo_unit[ price_rule == 1]
price_desejado[ price_rule == 2] <- ( 1 + 0)*lag_price_dej[ price_rule == 2]
price_desejado[ price_rule == 3] <- ( 1 + VAR(firm_mkup.movel.ms)[ price_rule == 3])*firm_custo_unit[ price_rule == 3]
price_desejado[ price_rule == 4] <- ( 1 + VAR(firm_mkup.movel)[ price_rule == 4] )*firm_custo_unit[ price_rule == 4]


firm_price_dej <- sum2( LAG(firm_ms,1) * LAG(firm_price,1) ) * ( 1 - LAG(firm_ms,1) ) + LAG(firm_ms,1) * firm_price_dej
...
name <- firm_leverage_meta <- function(firm_leverage_meta){

VAR(time_s)

firm_profit_rate_lag <- matrix(LAG(firm_leverage_meta,1),quantidade_firmas,1)
...
if(VAR(time_t) > 10){
  ...
  firm_profit_rate_lag <- matrix( rnorm(quantidade_firmas,mean=1,sd=0.05)*firm_profit_rate_lag ,quantidade_firmas,1)
  ...
  firm_leverage_meta <- firm_profit_rate_lag
  
  firm_leverage_meta[ ms == 0 | lev > 1 ] <- (rowSums2( matrix(firm_profit_rate_lag[selected,],quantidade_firmas,5))/5)[ ms == 0 | lev > 1 ]
  
}

if(VAR(time_t) == 1) firm_leverage_meta <- rep(0.5,quantidade_firmas)

firm_leverage_meta[ firm_leverage_meta < 0 ] <- 0
...
fixed.markup <- lag_firm_fixed_markup

#firm_ms <- LAG(firm_ms,1)

qual <- LAG(firm_quality,1)

qual2 <- LAG(firm_quality,2)

VAR(time_s) # Para rodar primeiro a atualização das lista de firmas

list_firms_selected2 <- matrix(list_firms_selected,quantidade_firmas,5)

profit_exp <- VAR(firm_exp_profit)

exp_demand <- VAR(firm_expdem)
exp_demand[exp_demand < 1] <- 1

custo_real <- LAG(firm_custo_unit,1)/LAG(agg_index_prices,1)

profit_exp <- profit_exp/LAG(agg_index_prices,1)

qual[list_firms_selected2[100,]]

qual3 <- qual/(rowSums2( matrix(qual[list_firms_selected2[1:100,]],quantidade_firmas,5))/5)


# P/Kp * Kp/ K * K/Kmaq * Kmaq = P/Kp * Kp/ K * K/Kmaq * vY

# PQ = wL + rK + R = w coef Q + r_t lvQ + r_p (1-l)vQ + R 
# P = w coef + r_t lv + r_p (1-l)v + R/Q
# (1 + m) w coef = w coef +  r_t l v + r_p (1-l)v + R/Q
# m =(1/w coef)*( r_t l v + r_p (1-l)v + R/Q)

assets_real <- LAG(firm_assets,1)/LAG(agg_index_prices,1)

prod_normal <- floor(VAR(firm_capacidade_producao)*(1/0.8))

#new_fix_mark <- ((0.2*(1-LAG(firm_leverage_meta,1)) + VAR(firm_interest)*LAG(firm_leverage_meta,1) )*2/prod_machine + VAR(firm_profit_rate)) * (1/(custo_real*VAR(firm_coef_tecn)))

new_fix_mark <- ((VAR(firm_profit_rate)*(1-LAG(firm_leverage_meta,1)) + VAR(firm_interest)*LAG(firm_leverage_meta,1) )*assets_real/prod_normal ) * (1/(custo_real*VAR(firm_coef_tecnico)))

custo_anterior <- LAG(firm_custo,1)
eff_demand <- LAG(firm_expdem,1)

UDC <- (custo_anterior/LAG(agg_index_prices,1))/eff_demand

UDC[ is.infinite(UDC)] <- mean(UDC[is.infinite(UDC) == FALSE])

assets_firms <- LAG(firm_assets,1)/LAG(agg_index_prices,1)

a.eff_demand <- assets_firms/eff_demand

a.eff_demand[ is.infinite(a.eff_demand)] <- mean(a.eff_demand[is.infinite(a.eff_demand) == FALSE])

# PY = wL + r(1-l)K + ilK
# P = UDC + r(1-l)K/Y + il K/Y
...
#if(VAR(time_t)> 10){
#
#  new_fix_mark <- ( VAR(firm_profit_rate)*(1-LAG(firm_leverage_meta,1)) + VAR(firm_interest)*LAG(firm_leverage_meta,1) ) * a.eff_demand * (1/UDC)
#
#}else{
#  
#  new_fix_mark <- ( (VAR(firm_profit_rate)*(1-LAG(firm_leverage_meta,1)) + VAR(firm_interest)*LAG(firm_leverage_meta,1) )*1/(prod_machine*grau_utilizao_desejado) ) * (1/(custo_real*VAR(firm_coef_tecn)*grau_utilizao_desejado))
# 
#}

new_fix_mark <-  ( VAR(firm_profit_rate)*(1-LAG(firm_leverage_meta,1)) + VAR(firm_interest)*LAG(firm_leverage_meta,1) ) * 1/(prod_machine*grau_utilizao_desejado) * (1/(custo_real*VAR(firm_coef_tecnico)*grau_utilizao_desejado))

fixed.markup <- 0.8 * fixed.markup + 0.2 * new_fix_mark

#new_fix_mark <- (1*qual3*(1-LAG(firm_leverage_meta,1)) + VAR(firm_interest)*LAG(firm_leverage_meta,1) )*(LAG(firm_assets,1)/LAG(agg_index_prices,1))*(1/(custo_real*VAR(firm_coef_tecn)))

#for(i in 1:quantidade_firmas){

#  id <- list_firms_selected2[i,]

#pond_rec <- rec_ope[id]/sum2(rec_ope[id])
#pond_rec[ pond_rec < 0] <- 0

#pond_total <- (firm_ms[id]/sum2(firm_ms[id]) + pond_rec)/2

#pond_total[pond_total < 0] <- 0

#pond_total <- pond_total/sum2(pond_total)


#fixed.markup[i] <- sum2(pond_total *(1 + 0.2*( qual[i]/qual[id] -1)) * prices[id] )/(cu[i]*coef[i]) - 1

#fixed.markup[i] <- (1 + parameter.adjust.markup*( qual[i]/sum2(qual) - qual2[i]/sum2(qual2) )) * fixed.markup[i]


#}


firm_fixed.markup <- fixed.markup
...
fixed.markup <- LAG(firm_fixed.markup,1)

salario_real <- LAG(firm_custo_unit,1)/LAG(agg_index_prices,1)

salario_real <- LAG(firm_custo_unit,1)

UDC <- LAG(firm_custo,1)/LAG(firm_expdem,1)/LAG(agg_index_prices,1)

UDC[ is.infinite(UDC)] <- mean(UDC[is.infinite(UDC) == FALSE])

FC <- LAG(firm_fb,1)/LAG(agg_index_prices,1)/LAG(firm_expdem,1)

FC[ is.infinite(FC) | is.nan(FC) ] <- (LAG(firm_fb,1)/LAG(firm_assets,1)/LAG(agg_index_prices,1))[ is.infinite(FC) | is.nan(FC)]

if( VAR(time_t) > 10 ){
  
  UDC[ UDC/mean(UDC) < 0.1 | UDC/mean(UDC) > 10 ] <- mean(UDC[ UDC/mean(UDC) > 0.1 & UDC/mean(UDC) < 10 ])
  
  new_fix_mark <-  ( VAR(firm_profit_rate)*2/(grau_utilizao_desejado*prod_machine) + FC ) * 1/UDC
  
  CUSTO_FIRMA <- LAG(firm_custo,1)
  CUSTO_FIRMA[ CUSTO_FIRMA == 1] <- mean(CUSTO_FIRMA[ CUSTO_FIRMA != 1])
  
  
  UC <- LAG(firm_wage_ref)*LAG(firm_coef_tecnico)/0.8   # wL = w coef Y
  vendas <- ceiling(LAG(firm_capacidade_producao,1)/0.8)
  CT <- UC * vendas
  
  new_fix_mark <-  ( LAG(firm_profit_rate,1)*(1- LAG(firm_leverage_meta,1))*LAG(firm_assets,1) + LAG(firm_fb,1) )/CT
  
}else{
  
  new_fix_mark <-  ( VAR(firm_profit_rate)*(1- LAG(firm_leverage_meta,1)) + VAR(firm_interest)*LAG(firm_leverage_meta,1)) *
    2/(grau_utilizao_desejado* prod_machine) * (grau_utilizao_desejado/salario_real) * VAR(firm_coef_tecnico)
  
}

#new_fix_mark <- rep(0.25,quantidade_firmas)

fixed.markup <- 0.6 * fixed.markup + 0.4 * new_fix_mark
...
if(VAR(time_t) == tinit){
  ...
  lag_firm_fixed_markup <- rep(0.6,quantidade_firmas)
...
if(VAR(time_t) > 100){
    ...
    lag_firm_fixed_markup[ price_rule == 1 ] <- ( 1 + var.markup.ms* ( LAG(firm_ms,1)[ price_rule == 1 ]-LAG(firm_ms,2)[ price_rule == 1 ] )) *
      lag_firm_fixed_markup[ price_rule == 1 ]
    
    lag_firm_fixed_markup[ price_rule == 2 ] <- ((1 + var.markup.ms* ( LAG(firm_quality,1)/mean(LAG(firm_quality,1)) - 
                                                                        LAG(firm_quality,2)/mean(LAG(firm_quality,2)) ) ) * 
                                                   lag_firm_fixed_markup)[ price_rule == 2 ]
    
    
    # lag_firm_fixed_markup[ price_rule == 3 ] <- ( r_implicit[ price_rule == 3 ] * (1 - LAG(firm_leverage_meta,1)[ price_rule == 3 ]) + LAG(firm_interest,1)[ price_rule == 3 ] * LAG(firm_leverage_meta,1)[ price_rule == 3 ] ) *
    #   1/grau_utilizao_desejado * prod_machine *
    #   LAG(firm_price,1)[ price_rule == 3 ]/( LAG(firm_wage_ref,1)[ price_rule == 3 ] * LAG(firm_coef_tecnico,1)[ price_rule == 3 ] ) - 1
    ...
    r_implicit <- rep( ( 1 + ( 1 + LAG(cb_ir,1))^4 - 1 )^(1/4) - 1 + 0.05 , 200 )
   
    profit_rate_realized[ is.infinite(profit_rate_realized) ] <- r_implicit[ is.infinite(profit_rate_realized) ]
    
    dif <- r_implicit[ price_rule == 3 ] - profit_rate_realized[ price_rule == 3 ] 
    
    dif[ dif < -0.3  ] <- -0.3
    dif[ dif > 0.3  ] <- 0.3
    
    lag_firm_fixed_markup[ price_rule == 3 ] <- ( 1 + var.markup.ms * ( dif )) *
      lag_firm_fixed_markup[ price_rule == 3 ]
    
    
    cond4 <- abs(firm_estoque_desejado_perc -
                   LAG(firm_estoque,1)/LAG(firm_expdem,1)) > 0.025
    
    lag_firm_fixed_markup[ price_rule == 4 & cond4 ] <- ( 1 + var.markup.ms * ( firm_estoque_desejado_perc[ price_rule == 4 & cond4 ] -
                                                                          LAG(firm_estoque,1)[ price_rule == 4 & cond4 ]/LAG(firm_expdem,1)[ price_rule == 4 & cond4 ]  ) ) *
      lag_firm_fixed_markup[ price_rule == 4 & cond4 ]
    
    lag_firm_fixed_markup[ is.infinite(lag_firm_fixed_markup) ] <- LAG(firm_fixed.markup,1)[ is.infinite(lag_firm_fixed_markup) ]
    
    lag_firm_fixed_markup[ lag_firm_fixed_markup < 0] <- 0.01
    
    #lag_firm_fixed_markup[ price_rule == 4 ] <- (1 + rnorm(quantidade_firmas,0,0.03)[ price_rule == 4 ] ) * lag_firm_fixed_markup[ price_rule == 4 ] 
...
name <- atualizar_price <- function(firm_price,firm_price_dej){

effect_demand <- VAR(firm_expdem)

price_index <- sum2( LAG(firm_price,1) * LAG(firm_ms,1) )

price_desired <- VAR(firm_price_dej)
current_price <- LAG(firm_price,1)

diff.var.price <- ( - price_desired + LAG(firm_price,1) )/price_index

betas_demand_esperada <- rep(0,quantidade_firmas)

#demand_esperada_com_var_price <-  VAR(firm_expdem) * ( 1/( 1 - betas_demand_esperada *( diff.var.price ) )) 

#demand_esperada_com_var_price <- VAR(firm_expdem) * ( 1 + ( ( 0.2/(1 + exp(25 * diff.var.price) )  ) - 0.2/2 ) )

change_prices_demand_effect <- rep(1,quantidade_firmas)

# for(i in 1:quantidade_firmas){
# 
#   if( diff.var.price[i] > 0 ){
#     
#     change_prices_demand_effect[i] <- 1/(1 +  betas_demand_esperada[i] * ( diff.var.price[i] ))
#     
#   }else{
#     
#     change_prices_demand_effect[i] <- 1/(1 +  (betas_demand_esperada[i] - 3.5) * ( diff.var.price[i] ))
#     
#   }
# }

# demand_esperada_com_var_price <- VAR(firm_expdem) * change_prices_demand_effect
# 
# cost_unit <- VAR(firm_coef_tecn) * VAR(firm_wage_ref)
# 
# #(price_desired - cost_unit)/(current_price - cost_unit) > effect_demand/demand_esperada_com_var_price
# 
# lucro_esperado_c_novo_preco <- price_desired * demand_esperada_com_var_price - VAR(firm_coef_tecn) * demand_esperada_com_var_price * VAR(firm_wage_ref)
# lucro_corrent <- current_price * effect_demand - VAR(firm_coef_tecn) * effect_demand * VAR(firm_wage_ref)
# 
# change_price <- rep(0,quantidade_firmas)
# change_price[  lucro_esperado_c_novo_preco > lucro_corrent ] <- 1
# 
# firm_price <- current_price
# firm_price[  lucro_esperado_c_novo_preco > lucro_corrent ] <- price_desired[  lucro_esperado_c_novo_preco > lucro_corrent ]


#firm_price <- LAG(firm_price,1)
#price_desejado <- VAR(firm_price_dej)
#threshold <- VAR(firm_threshold)

#firm_price[ abs(firm_price/price_desejado - 1) > threshold ] <- price_desejado[ abs(firm_price/price_desejado - 1) > threshold ]

firm_price <- VAR(firm_price_dej)
...
dividends <- 1 * ( (LAG(firm_leverage_meta,1) - LAG(firm_leverage,1)  ) ) * LAG(firm_assets,1) + dividends
...
capitalist_riqueza <- VAR(capitalist_disposable_income) - VAR(capitalist_consumo)  + dividendos +
  (sum(VAR(firm_demand_disp)) - sum2(VAR(firm_vendas)*VAR(firm_price))) + LAG(capitalist_riqueza,1) - 
  0.5*sum2(ifelse( LAG(firm_leverage,1) > 1 , LAG(firm_debt_stock,1), 0 ))
...
# 
# gov_consumption <- ( 1 + reac_gov*(( LAG(agg_unemployment_rate,1) / meta_emprego -1 )) - reac_gov_infla * ((LAG(agg_inflation,1)/ meta_inflacao - 1))) * gov_consumption_base 
# 
...
name <- atualizar_gov_deficit <- function(gov_tributos,gov_expense,cb_ir,gov_debt,cb_profit){
  ...
  gov_deficit <- - taxes - profit_central_bank + gover_expenses + basic_interest_rate*gov_debt_stock
...
#effect.price.mk <- (1/(1- market.inertia)) * (1/quantidade_firmas) * (1/10)
...
#meta_inflação = (1 + infla_indexation * meta_inflacao) * ( 1 + (1 - txa_de)*runif(1,0,par_var_salario_max)) - 1

#(1 + meta_inflacao)/(1 + infla_indexation * meta_inflacao) - 1 = (1 - txa_de)*runif(1,0,par_var_salario_max)

#((1 + meta_inflacao)/(1 + infla_indexation * meta_inflacao) - 1)/mean(runif(100000,0,par_var_salario_max)) # = 1 - txa_de

# Taxa de desemprego de equilíbrio
#1 - ((1 + meta_inflacao)/(1 + infla_indexation * meta_inflacao) - 1)/mean(runif(100000,0,par_var_salario_max))
...
ex_demand <- agg_supply - pro_consumir_wage * (1 - gov_ir[1,1]) * ( wage * quantidade_workers * taxa_inicio + (1 - taxa_inicio) * quantidade_workers * gov_unem_ben_init) # Consumo do governo + cpaitalist

goverment_init_expense <- ex_demand/10

capitalist_renda_disponivel_init <- (1 - gov_ir[1,1]) * (agg_supply - wage * quantidade_workers * taxa_inicio)
...
# Debbug. Se houver NA ou Inf, para
# Recriar matriz toda vez que refaz simu
...
#firm_custo/(firm_capacidade_producao*grau_utilizao_desejado)

#RESULTADOGA <- X

#X <- c(0.237275 ,0.2800863, 0.4838673, 0.20104, 0.4393784, 0.4228248, 0.4801497,
#       227.2243, 0.009738818, 0.6683235, 0.05092512, 0.2687403)

#X <- RESULTGA

#dif_salarial_emp_des <- X[1]
#porc_gasto_PD <- X[2]
#infla_indexation <- X[3]
#pro_consumir_wealth <- X[4]
#pro_consumir_dividends <- X[5]
#par_rate_deposits <- X[6]
#par_rate_emprestimos <- X[7]
#firmk_lb_prod <- X[8]
#r <- X[9]
#inertia_cb <- X[10]
#par_var_salario_max <- X[11]
#parameter_trab_indireto <- X[12]
#gov_inertia <- X[13]
#par_exp_firm_qual <- X[14]
#par.pas.dem <- X[15]
#reac_gov <- X[16]
...
par_var_salario_max <- (1.04)^(1/4) - 1

par_reac_infla <- 0.2
par_reac_emprego <- 0.1
...
max_readjustment_wage_ref <- 0.05
...
resultados <- simulationMULTABM(2,DEBBUG=0)
...
setwd("C:\\Users\\Pichau\\OneDrive\\Documentos\\R\\Teste GIt\\arquives")
load("resultados_rigid_price.Rbin")
...
for(y in 1:m){
  ...
  cov_simuls[y,] <- cor( resultados[[y]]$agg_inflation[1,800:periodos_simulacao] , freq_simuls[y,799:(periodos_simulacao-1)])
  
  
}


sd(freq_change_simuls)


1/mean(freq_change_simuls) * 3


plot(apply(freq_simuls,MARGIN=2,mean))
...
corr <- corr.function3.bk(resultados,GDP = c("agg_gdp"),Nomes = c("agg_gdp","agg_consumption","agg_investment","gov_consumption_real","agg_unemployment_rate",
                                                       "agg_capacity_util","agg_labor_prod","agg_inflation","agg_var_real_wage","cb_ir"),
                  Titulo=c("gdp","con","invest","gov_cons","unem","util","prod","inflation","real_Wage","interest"),start=ceiling(periodos_simulacao*3/4),periodos=6,
                  LAGS=c(1,1,1,1,0,0,1,0,0,0,0)) # Lags para definir o log de quem deve ser tirado
...
####### Quantidade de firmas que alteraram de estratégia ########
  
  matrix_firms_change_strat <- matrix(0,m,periodos_simulacao)

  
for(i in 1:m){
  for(t in 100:periodos_simulacao){
    for(f in 1:quantidade_firmas){
      
      if( resultados[[i]]$firm_price_rule[f,t] != resultados[[i]]$firm_price_rule[f,t-1] ){ matrix_firms_change_strat[m,t] <- 1 + matrix_firms_change_strat[m,t] }
      
    }
  }
}
  
  apply(matrix_firms_change_strat,MARGIN=2,mean)/quantidade_firmas
  
  
#####
  
  pos <- resultados[[1]]$firm_price_rule[,998] == 2
  
  hist( resultados[[1]]$firm_price[pos,998]/ resultados[[1]]$firm_price[pos,997] - 1 ,20)
  
  
  
  
####### Participação das estratégias e lucratividade #######

  matrix_part <- matrix(0,5,periodos_simulacao)
  
  matrix_profit <- matrix(0,5,periodos_simulacao)
  
  
  for(i in 1:5){
    
    for(t in 1:periodos_simulacao){
      
      matrix_part[i,t] <- sum(resultados[[1]]$firm_price_rule[,t] == i)
      
      k_stock <- resultados[[1]]$firm_expdem[,t]/(prod_machine * grau_utilizao_desejado) * resultados[[1]]$firmk_price[,t]
      
      matrix_profit[i,t] <- sum((resultados[[1]]$firm_profit[,t])[ resultados[[1]]$firm_price_rule[,t] == i ])/(sum(k_stock[ resultados[[1]]$firm_price_rule[,t] == i ]))
  ...
  plot(matrix_part[1,],type="l",ylim=c(0,200))
  lines(matrix_part[2,],col="red")
  lines(matrix_part[3,],col="blue")
  lines(matrix_part[4,],col="green")
  lines(matrix_part[5,],col="grey")
  
  
  df <- data.frame( matrix_profit[1,],matrix_profit[2,],matrix_profit[3,],matrix_profit[4,],1:length(matrix_profit[4,]))
  
  names(df) <- c("MS","Qual","Profit","Demand","Seq")
  

 graph_profit_rate_heuristics <-  ggplot(df) +
    geom_line(aes(x=Seq,y=MS,col="MS"),col="black",size=0.8) +
    geom_line(aes(x=Seq,y=Qual,col="Qual"),size=0.8) +
    geom_line(aes(x=Seq,y=Demand,col="Dem"),size=0.8) +
    geom_line(aes(x=Seq,y=Profit,col="Prof"),size=0.8) +
    ylim(0,0.3) +
    xlim(500,periodos_simulacao) +
    ylab("Profit rate") +
    xlab("Time") +
    theme(text = element_text(size=10),legend.title = element_blank()) +
    theme_light() +
 ...
 ggsave(graph_profit_rate_heuristics, filename = "graph_profit_rate_heuristics.png", dpi = 300, type = 'cairo',
        width = 16, height = 16*0.6, units = 'cm')
  
  
  
  
  plot(matrix_profit[1,],type="l",ylim=c(0,1)) # Market-share
  lines(matrix_profit[2,],col="red") # Quality
  lines(matrix_profit[3,],col="blue") #  Profit-target
  lines(matrix_profit[4,],col="green") # Demanda
  lines(matrix_profit[5,],col="grey") # Random
  
  
  
##### Gráficos  ######
  
  # graph.function(results=resultados,c("agg_gdp","agg_unemployment_rate","agg_inflation","agg_function_dist","firm_fixed.markup","firm_leverage"),
  #                Titulo=c("Aggregate GDP","Unemployment rate","Aggregate inflation","Agg.Func.Dist","Ave.Eff.Markup","Ave.Firm.Leverage"),start=periodos_simulacao-500,
  #                LOG=c(1,0,0,0,0,0,0))
  ...
  graph.function(results=resultados,c("agg_inflation"),
  ...
  graph.function(results=resultados,c("agg_unemployment_rate"),
                 Titulo=c("Ave.Unem rate"),start=750,
                 LOG=c(0))
  
  graph.function(results=resultados,c("agg_empresas_falidas"),
...
##### Gráficos dos ciclos das variáveis macroeconômicas####
  ...
  gdp_graph <- graph.function(results=resultados,c("agg_gdp"),
                 Titulo=c("Ave.Agg.GDP (log)"),start=750,
                 LOG=c(1)) 
  
  graphcorr2 <- grid.arrange(g,cycle_graph,ncol=2)
...
#### Firms market-share #####
  ...
  firms_market_share_graph <- ggplot(data=df,aes(growth)) +
    ...
    stat_function(fun = dnorm, args = list(mean=med(df$growth), sd=sd(df$growth)),col="blue",size=2) +
  ...
  df_g <- data.frame(c(resultados[[1]]$firm_ms[,1000]/resultados[[1]]$firm_ms[,1000-4] -1,
                       resultados[[1]]$firm_ms[,1000-8]/resultados[[1]]$firm_ms[,1000-12] -1))
  
  
  names(df_g) <- "growth"
  
  
  plot(df$growth,df_g$growth)
  
  lm(df_g$growth ~ df$growth)
  
  
  
  names(df) <- "growth"
  
  ggplot(data=df,aes(growth)) +
  ...
  df <- data.frame(c(resultados[[1]]$firm_vendas[,1000]/resultados[[1]]$firm_vendas[,(1000-4)] - 1,
                     resultados[[1]]$firm_vendas[,1000-4]/resultados[[1]]$firm_vendas[,(1000-4-4)] - 1,
                     resultados[[1]]$firm_vendas[,1000-8]/resultados[[1]]$firm_vendas[,(1000-8-4)] - 1))
  
  df <- data.frame(c(resultados[[1]]$firm_vendas[,1000],
                     resultados[[1]]$firm_vendas[,1000-4],
                     resultados[[1]]$firm_vendas[,1000-8],
                     resultados[[1]]$firm_vendas[,1000-12]))
  
  names(df) <- "growth"
  
  df <- data.frame(log(df$growth) - med(log(df$growth)))
...
#### Investment spikes ####
  ...
  plot(matrix_graphs[2,]/quantidade_firmas,ylim=c(0,1),type="l")
    ...
    ggsave(graphcorr1, filename = "ms_spikes.png", dpi = 300, type = 'cairo',
           width = 16, height = 16*0.6, units = 'cm')
...
#### Estabilidade das séries temporais  #####
